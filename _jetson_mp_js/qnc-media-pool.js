/* Media Pool — fiksni player/transkript gore, skrol timeline redovi dolje */
...
(function (QNC) {
  ...
  function clipNote(clipId, c) {
    ...
    if (note) return note;
    const parts = [];
    if (c.transferred) parts.push('na Jetsonu');
  ...
  function thumbUrl(clipId, seek) {
    return QNC.filmstrip.thumbUrl(clipId, seek, { thumbRev: pool.thumbRev });
  }

  /** Koliko 112px sličica stane u traku (bez početne lijevo). */
  function stripSlotCount() {
    const rows = $(ROWS_ID);
    const strip =
      rows?.querySelector('[data-qnc-panel="' + QNC.filmstrip.PANEL_ID + '"] [data-qnc-slot="frames"]') ||
      rows?.querySelector('.thumbnail-strip');
    const ws = rows?.closest('.pool-workspace');
    const fallbackWidth =
      ws && ws.clientWidth > 0 ? ws.clientWidth - 220 - THUMB_W - 64 - 36 : 0;
  ...
  function beginLiveTranscript(clipId) {
    const pid = QNC.getProjectId ? QNC.getProjectId() : 'default';
  ...
  function updatePlayerChrome() {
    ...
    const txKey = c?.clip_id && (QNC.getProjectId ? QNC.getProjectId() : 'default') + '::' + c.clip_id;
  ...
  function applyVirtualShots(nextVirtualShots, options) {
    ...
    const changed = force || sig !== pool.lastVirtualSignature;
    pool.virtualShots = nextVirtualShots || [];
    if (changed) {
      pool.lastVirtualSignature = sig;
      renderVirtualShots();
    }
  }
  ...
  function filmstripPanelForRow(row, clipId) {
    if (!row) return null;
    return (
      row.querySelector('[data-qnc-panel="' + QNC.filmstrip.PANEL_ID + '"]') ||
      row.querySelector('[data-qnc-component="' + QNC.filmstrip.PANEL_ID + '"]')
    );
  }

  function syncFilmstripForClip(clip, rowEl) {
    const row = rowEl || document.querySelector('.timeline-row[data-clip-id="' + CSS.escape(clip.clip_id) + '"]');
    const panel = filmstripPanelForRow(row, clip.clip_id);
    ...
    QNC.filmstrip.update(panel, {
      ...
      thumbUrl: (clipId, sec) => thumbUrl(clipId, sec),
      formatDuration,
  ...
  function rowHtml(c) {
    ...
    return (
      ...
      QNC.esc(note) +
      '</div></div>' +
      '<div data-qnc-component="media_pool.filmstrip-viewer" data-qnc-variant="inline"></div>' +
  ...
  async function render() {
    ...
    if (!pool.clips.length) {
      rows.innerHTML = '<p class="grid-empty muted">Potvrdi clipove iz Ingest taba</p>';
      updateUi();
      return;
    }
  ...
  function applyTimelinesFromClips(clips) {
    clips.forEach((c) => {
  ...
  function reconcileRows(nextClips, nextVirtualShots) {
    const rows = timelineRowsEl();
    const oldIds = ids();
    const oldById = new Map(pool.clips.map((clip) => [clip.clip_id, clip]));
    ...
    if (wasEmpty || removed || orderChanged) {
      render().catch((e) => QNC.setBox('Pool: ' + e.message, 'err'));
      return;
    }
    ...
    for (const clip of nextClips) {
      ...
      const timelineChanged = timelineKey(old) !== timelineKey(clip);
  ...
  async function loadPool() {
    ...
    const d = await QNC.api(
      'GET',
      '/api/media-pool/clips?project_id=' + encodeURIComponent(pid)
    );
    if (d.project_id && QNC.setActiveProjectId) {
      QNC.setActiveProjectId(d.project_id);
    }
    ...
    reconcileRows(nextClips, nextVirtualShots);
  ...
  QNC.tabs.register({
    ...
    init() {
      ...
      QNC.bus.on('project:changed', () => {
        pool.lastSignature = '';
        pool.lastVirtualSignature = '';
        pool.clips = [];
        loadPool().catch((e) => QNC.setBox('Pool: ' + e.message, 'err'));
      });
      render();
